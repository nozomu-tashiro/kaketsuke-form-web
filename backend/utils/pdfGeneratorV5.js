...
class PDFGeneratorV5 {
  ...
  async fillPage(pdfDoc, pageIndex, formData, font, copyType) {
    ...
    try {
      ...
      // Box 2: 電話番号
      // X座標: 200, Y座標(下から): 169, 最大幅: 150pt, 配置: 左寄せ・上寄せ
      if (agentInfo.phone) {
        const maxWidth = 150;
      ...
      // Box 3: 販売店コード
      // X座標: 465, Y座標(下から): 209, 最大幅: 85pt, 配置: 左寄せ・上寄せ
      if (agentInfo.code) {
        const maxWidth = 85;
      ...
      // Box 4: 担当者名
      // X座標: 465, Y座標(下から): 200, 最大幅: 85pt, 配置: 左寄せ・上寄せ
